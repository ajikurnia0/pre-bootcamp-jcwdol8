...
export const deletePost = async (req, res) => {
    ...
    try {
        ...
        if (postedBy.postedBy == req.user) {
            ...
            res.status(400).send({
                message: 'Cannot delete somebody elses post.'
            })
        }
    } catch (err) {
        throw err
...
export const editPostCaption = async (req, res) => {
    try {
        ...
        if (postedBy.postedBy == req.user) {
            ...
            res.status(404).send({
                isError: true,
                message: 'Cannot edit a post if it is not yours.',
                data: true
            })
        }
    } catch (err) {
...
export const getAllPosts = async (req, res) => {
    ...
    try {
        const posts = await Post.find().populate('postedBy').populate({ path: 'likes', select: 'userName profilePic' }).sort({ createdAt: -1 })
